...
// get all un-changed credentials
...
// change credential for verified credentials
...
export const getAllTransacrtion = async (req, res) => {
	try {
		const transactions = await prisma.transaction.findMany({
			where: { isPaid: true },
			orderBy: { createdAt: "desc" },
...
// controller for marking withdrawal is paid
export const markWithdrawlAsPaid = async (req, res) => {
	try {
		const { id } = req.params;
