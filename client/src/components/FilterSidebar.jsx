import { ChevronDown, Filter, FilterIcon, X } from 'lucide-react'
  ...
  return (
    <div className={`${showFilterPhone ? "max-sm:fixed" : "max-sm:hidden"} max-sm:inset-0 z-100 max-sm:h-screen max-sm:overflow-scroll bg-white rounded-lg shadow-sm border border-gray-200 h-fit sticky top-24 md:min-w-[300px]`}>
        ...
        <div className='p-4 border-b border-gray-200'>
            <div className='flex items-center justify-between'>
                ...
                <div className='flex items-center gap-2'>
                    <X onClick={onCleraFilters} className='size-6 text-gray-500 hover:text-red-700 p-1 hover:bg-gray-100 rounded transition-colors cursor-pointer'/>
        ...
        <div className='p-4 space-y-6 sm:max-h-[calc(100vh-200px)] overflow-y-scroll no-scrollbar'>
            {/* Search Bar */}
            <div className='flex items-center justify-between'>
                <input type="text" placeholder='Search by username, platform, niche, etc.' className='w-full text-sm px-3 py-2 border border-gray-300 rounded-md outline-gray-500' onChange={onChangeSearch} value={search} />
            </div>
                ...
                {expandedSections.platform && (
                    <div className='flex flex-col gap-2'>
                        {platform.map((platform)=>(
                            <label className='flex items-center text-gray-700 text-sm gap-2' key={platform.value}>
                                <input type="checkbox" 
