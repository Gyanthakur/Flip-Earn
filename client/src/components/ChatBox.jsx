...
const ChatBox = () => {
    ...
    const fetchChat = async () =>{
       try {
         ...
         const {data} = await axios.post(`${backendUrl}/api/chat`, {listingId: listing.id, chatId}, {headers: {Authorization: `Bearer ${token}`}})
    ...
    const handleSendMessage = async(e)=>{
        ...
        try {
            setIsSending(true)
            const token = await getToken();
            const {data} = await axios.post(`${backendUrl}/api/chat/send-message`, {chatId: chat.id, message: newMessage}, {headers: {Authorization: `Bearer ${token}`}})
        ...
        <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-2xl h-screen sm:h-[600px] flex flex-col'>
            ...
            <div className='bg-gradient-to-r from-indigo-600 to-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-between'>
                <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-sm text-indigo-100 truncate'>{user.id === listing?.ownerId ? `Chating with buyer (${chat?.chatUser?.name || 'Loading...'})` : `Chatting with seller (${chat?.ownerUser?.name || 'Loading...'})`}</p>
