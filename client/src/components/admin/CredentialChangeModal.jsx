...
const CredentialChangeModal = ({ listing, onClose }) => {
    ...
    const fetchCredential = async () => {
        try {
            ...
            const {data} = await axios.get(`${backendUrl}/api/admin/credential/${listing.id}`, {headers: {Authorization: `Bearer ${token}`}})
    ...
    const changeCredential = async () => {
        try {
            const token = await getToken();
            const {data} = await axios.put(`${backendUrl}/api/admin/change-credential/${listing.id}`, {newCredential, credentialId: credential.id}, {headers: {Authorization: `Bearer ${token}`}})
