...
const ListingDetails = () => {
	...
	const purchaseAccount = async () => {
		try {
			...
			const { data } = await axios.get(
				`${backendUrl}/api/listing/purchase-account/${listing.id}`,
	...
	const loadChatBox = () => {
		...
		if (user.id === listing.ownerId)
			return toast("You can't  chat with your own listings");
	...
	return listing ? (
		<>
			<div className="mx-auto min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
				<button
					onClick={() => navigate(-1)}
					className="flex items-center gap-2 text-slate-600 py-4"
				>
				...
				<div className="flex items-start max-md:flex-col gap-10">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 text-sm">
								...
								<div>
									...
									<p className="flex items-center font-medium">
										{" "}
										<MapPin className="size-4 text-gray-400 mr-1" />{" "}
										{listing.country}
									</p>
								</div>
								<div>
					...
					<div className="bg-white min-w-full md:min-w-[370px] rounded-xl border border-gray-200 p-5 max-md:mb-10">
						...
						<div className="flex items-center gap-3 mb-2">
							...
							/>
							<div>
								<p className="font-medium text-shadow-gray-800">
									{listing.owner?.name}
								</p>
