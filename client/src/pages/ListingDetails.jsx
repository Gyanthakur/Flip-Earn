...
const ListingDetails = () => {
  ...
  const loadChatBox = () => {
    if(!isLoaded || !user) return toast("Please login to chat with seller")
      if(user.id === listing.ownerId) return toast("You can't  chat with your own listings")
      ...
      <div className='flex items-start max-md:flex-col gap-10'>
            ...
            <div className='bg-white rounded-xl border border-gray-200 mb-5 overflow-hidden'>
              ...
              <div className='relative w-full aspect-video overflow-hidden'> 
                ...
                <div className='absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2'>
                  {images.map((_, index)=>(
                    <button onClick={()=> setCurrent(index)} key={index} className={`w-2.5 h-2.5 rounded-full ${current === index ? "bg-indigo-600" : "bg-gray-400"}`}/>
                  ))}
                </div>
          ...
          <div className='bg-white rounded-xl border border-gray-200 mb-5'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6 p-4 text-sm'>
              ...
              <div>
                ...
                <p className='flex items-center font-medium'>  <MapPin className='size-4 text-gray-400 mr-1'/> {listing.country}</p> 
              </div>
              <div>
        ...
        <div className='bg-white min-w-full md:min-w-[370px] rounded-xl border border-gray-200 p-5 max-md:mb-10'>
          ...
          <div className='flex items-center gap-3 mb-2'>
            <img src={listing.owner?.image} alt="seller images" className='size-10 rounded-full' />
            <div>
              <p className='font-medium text-shadow-gray-800'>{listing.owner?.name}</p>
