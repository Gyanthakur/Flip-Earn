...
const MyOrders = () => {
  ...
  const fetchOrders = async()=> {
    try {
      ...
      const {data} = await axios.get(`${backendUrl}/api/listing/user-orders`, {headers: {Authorization: `Bearer ${token}`}})
  ...
  if(loading){
    return (
      <div className='h-[b0vh] flex items-center justify-center'>
  ...
  if(!orders.length){
    return (
      <div className='px-4 md:px-16 lg:px-24 xl:px-32'>
        <div className='max-w-2xl mx-auto mt-14 bg-white rounded-xl border border-gray-200 text-center'>
          <h3 className='text-lg font-semibold'>No Orders Yet</h3>
          <p className='text-sm text-gray-500 mt-2'>You have not placed any orders yet. Browse the marketplace to find listings you like!</p>
        </div>
                ...
                <div className='mt-4 md:mt-0 pt-4'>
                  <div className='space-y-2'>
                    {credential.updatedCredential.map((cred)=>(
                      <div key={cred.name} className='flex items-center justify-between gap-3 bg-gray-50 rounded-md'>
                        <div>
                        ...
                        <div className='flex items-center gap-2'>
                          ...
                          className='px-2 py-1 text-xs bg-white border border-gray-200 rounded hover:shadow hover:text-green-500' title="Copy credential" >
