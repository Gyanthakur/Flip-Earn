import { ArrowDownCircleIcon, BanIcon, CheckCircle, CheckCircleIcon, Clock, CoinsIcon, DollarSign, DollarSignIcon, EditIcon, Eye, EyeIcon, EyeOffIcon, LockIcon, Plus, StarIcon, TrashIcon, TrendingUp, Users, WalletIcon, XCircleIcon } from 'lucide-react'
...
const MyListing = () => {
  ...
  const deleteListing = async(listingId) => {
    try {
      const confirm = window.confirm('Are you sure you want to delete this listing? if credential are changed, new credentials will be sent to your email')
      if(!confirm) return;
    ...
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 pt-8'>
      ...
      <div className='flex flex-col sm:flex-row justify-between gap-4 xl:gap-20 p-6  mb-10 bg-white rounded-xl border border-gray-200'>
        {[
          ...
          {label: 'Available', value: balance.withdrawn, icon: CoinsIcon},
        ].map((item, index)=>(
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {
            userListings.map((listing)=>(
              <div key={listing.id} className='bg-white rounded-l-2xl border border-gray-200 hover:shadow-lg shadow-gray-200/70 transition-shadow'>
                <div className='p-6'>
                              ...
                              <div className='bg-white text-gray-600 text-xs rounded border border-gray-200 p-2 px-3'>
                                ...
                                <button className='text-nowrap'>
                                  ...
                                  <span className={
                                    ...
                                    ? "text-green-600" : "text-indigo-600" : "text-pink-600" : "text-red-600"
                                  }>
                                    {
                                    listing.isCredentialSubmitted 
                                    ? listing.isCredentialVerified
