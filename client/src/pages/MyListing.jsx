...
const MyListing = () => {
	...
	const toggleStatus = async (listingId) => {
		try {
			...
			const data = await axios.put(
				`${backendUrl}/api/listing/${listingId}/status`,
				{},
	...
	const deleteListing = async (listingId) => {
		try {
			const confirm = window.confirm(
				"Are you sure you want to delete this listing? if credential are changed, new credentials will be sent to your email"
			);
			if (!confirm) return;
	...
	const markAsFeatured = async (listingId) => {
		try {
			...
			const token = await getToken();
			const data = await axios.put(
				`${backendUrl}/api/listing/featured/${listingId}`,
				{},
				{ headers: { Authorization: `Bearer ${token}` } }
			);
			dispatch(getAllUserListing({ getToken }));
				...
				{userListings.length === 0 ? (
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						{userListings.map((listing) => (
							<div
								key={listing.id}
								className="bg-white rounded-l-2xl border border-gray-200 hover:shadow-lg shadow-gray-200/70 transition-shadow"
							>
								<div className="p-6">
