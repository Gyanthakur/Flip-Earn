...
const Messages = () => {
  ...
  const fetchUserChats = async () => {
    try {
      ...
      const {data} = await axios.get(`${backendUrl}/api/chat/user`, {headers: {Authorization: `Bearer ${token}`}})
      setChats(data.chats);
      setLoading(false)
    } catch (error) {
    ...
    <div className='mx-auto min-h-screen px-6 md:px-16 lg:px-24 xl:px-32'>
      <div className='py-10'>
        ...
        <div className='relative max-w-xl mb-8'>
          ...
          <input type="text" placeholder='Search Conversation...' value={searchQuery} onChange={(e)=>setSearchQurey(e.target.value)} className='w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-indigo-500' />
        </div>
...

        {
          ...
          : filteredChats.length === 0 ? (
            <div className='bg-white rounded-lg shadow-xs border border-gray-200 py-16 text-center'>
              ...
              <p className='text-gray-600'>
                {setQuarter ? "Try a different search term" : 'Start a conversation by viewing a listing and checking "chat with Seller"'}
              </p>
            </div>
          )
          :
